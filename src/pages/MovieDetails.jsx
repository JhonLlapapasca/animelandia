...
export default function AnimeDetails() {
  ...
  let movie = getMovie(parseInt(params.movieId));
  return (
    <div>
      <Container className="my-2">
        ...
        <span>
          {" "}
          Estas viendo: <strong>{movie.title}</strong>
        </span>
      </Container>
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.1) 0%, rgba(0, 0, 0.5) 100%),url(${movie.background})`,
        }}
      >
        <Container className="mb-4">
          <h3 className={styles.title_anime}>{movie.title}</h3>
          <p className={styles.information_anime}>{movie.information}</p>
        </Container>
      </div>
